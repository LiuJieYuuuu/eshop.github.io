...
<body>
<c:choose>
	<c:when test="${param.ep_id !=null && product==null }">
		<%response.sendRedirect("ProductView?ep_id="+ request.getParameter("ep_id")); %>
	</c:when>
...
<div id="main" class="wrap">
	...
	<div id="product" class="main">
		<h1>${product.name }</h1>
		<div class="infos">
			<div class="thumb"><img src="images/product/${product.filename }"/></div>
			<div class="buy">
				<p>商城价：<span class="price">￥${product.price }</span></p>
				<p>库　存：${product.stock }</p>
				<p>描    述：${product.description }</p>
				<div class="button">
					<input type="button" name="button" value="" onclick="goBuy(1)" />
					<a href="Shopping?type=pv&ep_id=${product.id }&uri=<%=request.getRequestURL() %>">放入购物车</a>
				</div>
			</div>
			<div class="clear"></div>
		</div>
