...
function reImg()
{
	document.getElementById("veryCode").src="number.jsp?code=" + Math.random();
}
...
<c:choose>
	<c:when test="${code !=null }">
		<%session.removeAttribute("code");out.println("<script>window.onload=function(){reminder('验证码填写错误！');}</script>"); %>
	</c:when>
	<c:when test="${regist ==1 }">
		<%session.removeAttribute("regist");out.println("<script>location.replace('reg-result.jsp');</script>");  %>
	</c:when>
	<c:when test="${regist ==2 }">
		<%session.removeAttribute("regist");out.println("<script>location.replace('reg-result.jsp');</script>");  %>
	</c:when>
</c:choose>
...
<div id="register" class="wrap">
	<div class="shadow">
		...
		<div class="box">
			...
			<form id="regForm" method="post" action="Regist?type=register" onsubmit="return checkForm(this);">
				<table>
					<tr>
					...
					<tr>
						...
						<td><input class="text verycode" type="text" name="veryCode" onfocus="FocusItem(this)" onblur="CheckItem(this);" />
							<img id="veryCode" src="number.jsp" onclick="reImg()" /><span></span></td>
					</tr>
					<tr>
						<td></td>
...
<div id="dlg" class="easyui-dialog" title="提示" style="width:400px;height:200px;padding:10px"
			data-options="
				iconCls: 'icon-save',
				modal:true,
				closed:true,
